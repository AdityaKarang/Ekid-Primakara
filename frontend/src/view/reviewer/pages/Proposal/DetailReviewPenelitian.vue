    ...
    <v-card>
      <v-card-text>
        <v-row>
          ...
          <v-col cols="12" md="7">
            <iframe :src="getProposalUrl(dataLaporan.proposal)" frameborder="1" width="100%" height="500px"></iframe>
          </v-col>
        </v-row>
      </v-card-text>
    </v-card>
      ...
      <v-expansion-panels>
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            <v-data-table :headers="headerPenilaian" :items="dataNilai" item-class="p-0" hide-default-footer>
              <template slot="item" slot-scope="props">
                <template v-for="(indikator, i) in props.item.indikators">
                  <tr :key="i + '_' + indikator.name">
                    ...
                    <td>
                      <v-radio-group v-model="indikator.nilai" style="font-size: 12px" row>
                  <tr>
                    <td>
                      <v-radio label="4" color="green" value="4" size="sm"></v-radio>
                    ...
                    <td>
                      <v-radio label="2" color="indigo" value="2" class="ml-6"></v-radio>
                      <label for="" class="ml-6" width="60px">
                        {{ indikator.prefix_radio }} Cukup</label>
                    </td>
                    <td>
                      <v-radio label="1" color="orange" value="1" class="ml-6"></v-radio>
                      <label for="" class="ml-6" width="60px">
                        {{ indikator.prefix_radio }} Kurang</label>
                    </td>
                    <td>
                      <v-radio label="0" color="red" value="0" class="ml-6"></v-radio>
                      <label for="" class="ml-6" width="60px">
            ...
            <v-row class="mt-2">
              <v-col cols="12">
                <v-textarea name="input-catatan" label="Catatan" outlined v-model="inputCatatan" value=""></v-textarea>
              </v-col>
              <v-col cols="12">
                <div class="float-right">
                  <div class="my-2">
                    <v-btn small class="mr-2" color="success" @click="confirmasi = true"  :disabled="isSaveDisabled">Save</v-btn>
                  </div>
                  <div class="my-2"></div>
                </div>
              </v-col>
...
export default {
  ...
  methods: {
    getProposalUrl(berkas) {
      const baseUrl = "http://localhost:8000/storage/penelitian/proposal/";
      return baseUrl + berkas;
    },  
    // simpanData() {
    //   if (this.indikator.nilai !== null) {
    //     this.dataArray.push(this.indikator.nilai); // Menyimpan nilai ke dalam array
    //   }
    // },
    ...
    // // Fetch the data based on the item object (use appropriate API endpoint or method)
    // axios.get('http://localhost:8000/api/penelitian/' + item.aksi)
    //   .then((response) => {
    //   console.log("API Response:", response.data.data); // Log the API response
    ...
    onSaveData() {
      ...
      const dataPenilaian = {
        penilaian_id: item.aksi,
        kriteria_penilaian_id: this,
      ...
      axios.post('http://localhost:8000/api/data-penilaian', dataPenilaian)
        .then(response => {
          console.log('Data saved successfully!', response.data);
          // Handle any further actions after successful data saving
        })
        .catch(error => {
    ...
    //   // Loop melalui setiap indikator
    //   this.dataNilai.forEach(item => {
    ...
    //   axios.post('/penelitian-value', hasil)
    //     .then(response => {
    //       console.log('Data berhasil disimpan:', response.data);
    //       // Tambahkan logika atau notifikasi sukses di sini jika diperlukan
    //     })
    //     .catch(error => {
    //       console.error('Error:', error);
    //       // Tambahkan logika atau notifikasi error di sini jika diperlukan
    //     });
