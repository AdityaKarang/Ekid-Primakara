    ...
    <v-card>
      <v-card-text>
        <v-row>
          ...
          <v-col cols="12" md="7">
            <iframe :src="getProposalUrl(dataLaporan.laporanakhir.laporan_akhir)" frameborder="1" width="100%"
              height="500px"></iframe>
          </v-col>
        </v-row>
      </v-card-text>
...
export default {
  ...
  methods: {
    getProposalUrl(berkas) {
      const baseUrl = "http://localhost:8000/storage/pengabdian/laporan/";
    ...
    fetchData() {
      ...
      axios.get('http://localhost:8000/api/pengabdian/' + item.aksi)
        .then((response) => {
    ...
    simpanData() {
      const data = {
        dataPenilaian: {
          pengabdian_id: this.dataLaporan.id,
      ...
      axios.post('/add-nilai-pengabdian', data)
        .then(response => {
