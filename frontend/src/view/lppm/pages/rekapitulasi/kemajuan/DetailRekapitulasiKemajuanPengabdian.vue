            ...
            <tbody>
              ...
              <tr class="mt-2">
                ...
                <td>
                  <h5>
                    <a
                      v-if="dataLaporan.laporankemajuan.laporan_kemajuan !== ''"
                      :href="getUrl(dataLaporan.laporankemajuan.laporan_kemajuan)"
                      target="__blank"
                    >
                      :
      ...
      <div>
        <v-tabs v-model="activeTab">
          ...
          <v-tab-item v-for="(item, index) in proposalResults" :key="index">
            <v-card>
              <v-card-title>
                Hasil Penilaian Laporan Kemajuan
                <v-spacer></v-spacer>
              </v-card-title>
...
export default {
  ...
  mounted() {
    ...
    // this.dataLaporan.judul = this.$route.params.data.judul;
    // this.dataLaporan.tahunAkademik = this.$route.params.data.tahun_ajaran;
    // this.dataLaporan.semester = this.$route.params.data.semester;
    // this.dataLaporan.mitra = this.$route.params.data.mitra;
    // this.dataLaporan.bidangFokus = this.$route.params.data.bidangFokus;
    // this.dataLaporan.dana = this.$route.params.data.dana;
    // this.dataLaporan.anggotaDosen = this.$route.params.data.anggotaDosen;
    // this.dataLaporan.anggotaMahasiswa =
    //   this.$route.params.data.anggotaMahasiswa;

    // this.datalistPublikasi = this.$route.params.data.publikasi;
    this.$store.dispatch(SET_BREADCRUMB, [{ title: "Detail Kemajuan" }]);
  },
  methods: {
    fetchData() {
    ...
    getDataNilai(index) {
      ...
      const penelitianPromise = axios.get('http://localhost:8000/api/show-value-kemajuan/' + item.aksi);
