<template>
  <b-card>
    <div class="mt-3">
        ...
        <v-card class="">
          ...
          <v-card-text>
            <v-form>
              <v-row>
                <v-col md="6">
                  <v-text-field v-model="form.bentuk_dana" label="Bentuk Dana" required></v-text-field>
                </v-col>
              </v-row>
      ...
      <CustomCard title="Daftar Bentuk Dana" deskripsi="List Daftar Bentuk Dana Yang Telah Di Inputkan" :headers="headers"
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        { text: "No", value: "no" },
        { text: "Bentuk Dana", value: "bentuk_dana" },
  ...
  mounted() {
    this.getbentuk_danas();
    console.log(this.currentUser);
  },
  methods: {
    getbentuk_danas() {
      axios.get('http://localhost:8000/api/bentukdana').then(res => {
        this.contentTables = res.data.data.map((bf, index) => {
          return {
            no: index + 1,
            bentuk_dana: bf.bentuk_dana,
            detail: bf.id
          }
        })
        console.log()
    ...
    onClick() {
      axios.post('http://localhost:8000/api/bentukdana', {
        bentuk_dana: this.form.bentuk_dana,
      })
        .then(response => {
    ...
    editData(item) {
      console.log(item);
      this.form = { bentuk_dana: item.bentuk_dana };
      this.showEdit = true;
    ...
    onSave() {
      axios.put(`http://localhost:8000/api/bentukdana/${this.editItemId}`, {
        bentuk_dana: this.form.bentuk_dana
      }).then(res => {
    ...
    onDelete() {
      axios.delete(`http://localhost:8000/api/bentukdana/${this.deleteId}`)
        .then(response => {
          console.log(response.data);
...
<!-- methods: {
  ...
  editData(item) {
    console.log("test");
    this.showAdd = true;
    this.form.bentukdana = item.bentukdana;
  },
  deleteData(item) {
    this.confirmasiDelete = true;
  },
