<template>
  <div>
    ...
    <CustomCard title="Daftar Proposal Penelitian" deskripsi="List Daftar Proposal Penelitian Yang Telah Di Inputkan"
      :headers="headers" :contentTable="filterDataContentTable" :additionalData="additionalData" :isTopAction="false"
      :isEdit="false" :isDelete="false" :isDetail="false" @detailItem="detailData" @saveReviewers="saveReviewers">
        ...
        <v-dialog v-model="confirmasiRevisi" max-width="350">
          <template v-slot:activator="{ on, attrs }">
            <v-tooltip v-if="checkStatus(item.status, 'revisi')" left>
              <template v-slot:activator="{ on, attrs }">
                <v-icon color="yellow" class="mr-2" v-bind="attrs" v-on="on" @click="btnRevisi(item)">
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch(SET_BREADCRUMB, [{ title: "Proposal Penelitian" }]);
    this.fetchData();
    this.fetchReviewer();
  },
  methods: {
    ...
    fetchData() {
      ...
      const penelitianPromise = axios.get('http://localhost:8000/api/get-nilai-penelitian');
        ...
        .then(([reviewerRes, penelitianRes]) => {
          ...
          this.contentTables = penelitianRes.data
            ...
            .map((bf, index) => {
              ...
              let status = bf.status === "Submited" ? "Empty" : bf.status;
    ...
    uploadContract(item) {
      ...
      formData.append("kontrak", this.contractFile);
      formData.append("status", "Aproved");

      axios
        .post("http://localhost:8000/api/kontrak-penelitian/" + this.id, formData, {
          headers: {
    ...
    reject(item) {
      ...
      axios
        .post("http://localhost:8000/api/update-status-penelitian/" + this.id, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
