<template>
  <div>
    ...
    <CustomCard title="Daftar kamajuan Penelitian" deskripsi="List Daftar Kemajuan Penelitian Yang Telah Di Inputkan"
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      const penelitianPromise = axios.get('http://localhost:8000/api/get-nilai-kemajuan');
      ...
      Promise.all([reviewerPromise, penelitianPromise])
        .then(([reviewerRes, penelitianRes]) => {
          ...
          this.contentTables = penelitianRes.data
            ...
            }).filter(item => item !== null); // Filter untuk menghapus item dengan laporankemajuan null
    ...
    approve(item) {
      axios
        .get('http://localhost:8000/api/laporankemajuan/' + this.id)
        .then((response) => {
