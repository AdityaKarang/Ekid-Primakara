    ...
    <v-row justify="center">
      <v-dialog v-model="confirmasi" max-width="290">
        <v-card>
          ...
          <v-card-text>
            Pastikan anda mengisi data dengan benar, data yang sudah disubmit
      ...
      <v-card-text>
        <v-form>
          ...
          <v-row v-else>
            ...
            <v-col md="6">
              <v-text-field v-model="form.semester" label="Semester" required
                :disabled="checkDisable($route.params.data.status)"></v-text-field>
            </v-col>
            <v-col sm="12" md="12">
                ...
                <template v-slot:item="data">
                  ...
                  <template v-else>
                    <v-list-item-content>
                      ...
                      ></v-list-item-subtitle>
                      <v-list-item-subtitle
                        v-html="data.item.jabatan_fungsional"
                      ></v-list-item-subtitle>
                      <v-list-item-subtitle
            ...
            <!-- <v-col md="12" sm="12">
              <v-autocomplete v-model="form.anggotaMahasiswa" :items="DataMahasiswa" dense filled item-text="nama"
                item-value="nama" label="Anggota Penelitian Mahasiswa " multiple
                :disabled="checkDisable($route.params.data.status)">
                <template v-slot:selection="data">
                  <v-chip v-bind="data.attrs" :input-value="data.item.nama" close @click="data.select"
            ...
            <v-col cols="12">
                <v-text-field v-model="form.mahasiswa_1" label="Anggota Mahasiswa 1" required></v-text-field>
              </v-col>
              <v-col cols="12">
                <v-text-field v-model="form.mahasiswa_2" label="Anggota Mahasiswa 2" required></v-text-field>
              </v-col>
            <v-col md="12" cols="12">
              <v-select v-model="form.bidang_fokus" :items="bidangfokuss" label="Bidang Fokus"
          ...
          <b-button v-if="!checkDisable($route.params.data.status)" type="button" class="mr-2" variant="success"
            @click="onConfirmation" :disabled="checkDisable($route.params.data.status)">Draft</b-button>

          <b-button v-if="!checkDisable($route.params.data.status)" type="button" variant="primary" @click="confirmasi = true"
            :disabled="checkDisable($route.params.data.status)">Submit</b-button>
        </v-form>
      </v-card-text>
    </v-card>
...
export default {
    ...
    fetchData() {
      ...
      axios.get('http://localhost:8000/api/pengabdian/' + item.detail)
        .then((response) => {
          ...
          this.form.anggota_dosen_1 = response.data.data.anggota_dosen_1;
          this.form.mahasiswa_1 = response.data.data.mahasiswa_1;
          this.form.mahasiswa_2 = response.data.data.mahasiswa_2;
          ...
          this.form.proposal = response.data.data.proposal;
  ...
  .catch((error) => {
    ...
    onChangeDosenSelect(e) {
      ...
      if (this.searchDosen && this.searchDosen.toLowerCase() == "tambah dosen luar") {
        ...
        let data = this.Dosen.find((e) => {
          return e.name.toLowerCase() == this.searchDosen.toLowerCase();
        });
    ...
    dataArray(ar) {
      ...
      if (!ar) return [];
      ar.forEach((e) => {
        newAr.push({ label: "Sumber Dana", value: e.sumber_dana });
        newAr.push({ label: "Bentuk Dana", value: e.bentuk_dana });
        newAr.push({ label: "Jumlah Dana", value: e.jumlah_dana });
      });
      return newAr;
    },
