      ...
      <v-card-text>
        <v-form>
          ...
          <b-button v-if="!checkDisable($route.params.data.status)" type="button" variant="primary"
            :disabled="!isFormComplete || checkDisable($route.params.data.status)"
            @click="confirmasi = true">Submit</b-button>
        </v-form>
      </v-card-text>
    </v-card>
...
export default {
  ...
  methods: {
    ...
    getDataNilai(index) {
      ...
      const penelitianPromise = axios.get('http://localhost:8000/api/show-nilai-penelitian/' + item.detail);
    ...
    onClick() {
      ...
      axios.post('http://localhost:8000/api/penelitian/' + item, formData, {
        headers: {
