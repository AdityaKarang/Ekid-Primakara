                ...
                <td>
                  <h5>
                    <a v-if="dataLaporan.laporankemajuan.laporan_kemajuan !== ''"
                      :href="getUrl(dataLaporan.laporankemajuan.laporan_kemajuan)" target="__blank">
                      :
                ...
                <td>
                  <h5>
                    <a v-if="dataLaporan.laporanakhir.laporan_akhir !== ''"
                      :href="getUrl(dataLaporan.laporanakhir.laporan_akhir)" target="__blank">
                      :
                      <i class="flaticon-file-2 text-primary"></i>
...
export default {
      ...
      datalistPublikasi: [
        {
          ...
          judulPublikasi:
            "Pelatihan Digital Marketing UMKM Sokasi di Desa Tigawasa",
          namaMedia: "Metro Bali ",
          url: "https://metrobali.com",
        },
      ],
      dataLaporan: {
  ...
  async mounted() {
    ...
    this.$store.dispatch(SET_BREADCRUMB, [{ title: "Detail Pengabdian" }]);
  },
  methods: {
    async fetchData() {
      try {
        ...
        const responseMain = await axios.get('http://localhost:8000/api/get-akhir-kemajuan/' + item.detail);
